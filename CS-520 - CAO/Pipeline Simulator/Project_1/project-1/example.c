  ...
  *  Author :
  *  Madhan Thangavel (mthanga1@binghamton.edu)
...
static void push_stages(APEX_CPU* cpu) {
  ...
  // To find Instruction per cycle
  int ipc = cpu->code_memory_size/cpu->clock;
  printf("IPC : %d\n", ipc);
...
int main(int argc, const char * argv[]) {
  ...
  if (argc != 4) {
    ...
    fprintf(stderr, "Help : Usage %s <input_file> <func(eg: simulate Or display)> <num_cycle>\n", argv[0]);
    exit(1);
  }
  else {
    strcpy(func, argv[2]);
